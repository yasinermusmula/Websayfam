...
export default function Projects() {
  ...
  return (
    <div
      id="projects"
      className="w-11/12 content-center flex-wrap gap-20 mt-8 my-6 md:flex-col md:mt-48"
    >
      <img src={vector} className="ml-32 w-11/12" />
      <p
        id="projects"
        className="dark:text-[#AEBCCF] ml-32 my-8 text-5xl not-italic font-semibold text-[#1F2937]"
      >
        {ProjectData.title}
      </p>
      <div className=" flex md:flex-col">
        {ProjectData.projects.map((key) => (
          <div key={key.id} className="flex flex-col ml-32">
            ...
            <div className="dark:text-[#FFFFFF] flex flex-wrap w-full gap-[9px] pb-10 mt-3 text-[#6B7280]">
              {key.projectInfo}
            </div>
